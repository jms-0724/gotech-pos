        ...
        if($_FILES['addprod_photo']['error'] == 0){
            ...
            if(in_array($_FILES['addprod_photo']['type'],$typesallowed)){
                ...
                if(move_uploaded_file($_FILES["addprod_photo"]['tmp_name'], $targetPath)) {
                    $stmt = $conn->prepare("INSERT INTO tbl_inventory (prod_id, prod_name, prod_type, prod_quantity, prod_price, prod_photo) VALUES(?,?,?,?,?,?)");

                    if (!$stmt) {
                        echo "NO SQL Prepared";
                    }
            
                    $pId = $_POST['addprod_id'];
                    ...
                    $stmt->bind_param("ssssss", $pId, $pname, $ptype, $pquantity, $price, $pphoto); 
