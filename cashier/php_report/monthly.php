...
$pdf = new PDF();
$sql = "SELECT DATE_FORMAT(tbl_transaction.transact_date, '%Y-%m') AS 'year_month', SUM(tbl_transaction_details.total_price) AS 'Total Sales', SUM(tbl_transaction.total_quantity) AS 'Total Sold', AVG(tbl_transaction_details.total_price) AS 'Average Prices', AVG(tbl_transaction.total_quantity) AS 'Average Quantity' FROM tbl_transaction INNER JOIN tbl_transaction_details ON tbl_transaction.transact_id = tbl_transaction_details.transact_id GROUP BY MONTH(tbl_transaction.transact_date)";
