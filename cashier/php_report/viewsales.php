<?php
    ...
    if(isset($_POST['search'])) {
        $search = $_POST['search'];
        $stmt = $conn->prepare("SELECT tbl_transaction.transact_date, SUM(tbl_transaction_details.total_price) AS 'Total Sales', SUM(tbl_transaction.total_quantity) AS 'Total Sold', ROUND(AVG(tbl_transaction_details.total_price),2) AS 'Average Prices', ROUND(AVG(tbl_transaction.total_quantity),2) AS 'Average Quantity' FROM tbl_transaction INNER JOIN tbl_transaction_details ON tbl_transaction.transact_id = tbl_transaction_details.transact_id WHERE tbl_transaction.transact_date LIKE '%$search%' GROUP BY tbl_transaction.transact_date");
