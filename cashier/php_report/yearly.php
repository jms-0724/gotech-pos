...
$pdf = new PDF();
$sql = "SELECT DATE_FORMAT(tbl_transaction.transact_date, '%Y') AS 'year_month', SUM(tbl_transaction_details.total_price) AS 'Total Sales', SUM(tbl_transaction.total_quantity) AS 'Total Sold', ROUND(AVG(tbl_transaction_details.total_price),2) AS 'Average Prices', ROUND(AVG(tbl_transaction.total_quantity),2) AS 'Average Quantity' FROM tbl_transaction INNER JOIN tbl_transaction_details ON tbl_transaction.transact_id = tbl_transaction_details.transact_id GROUP BY YEAR(tbl_transaction.transact_date)";
...
$pdf->Cell(80);
$pdf->Cell(0, 10,'List of Monthly Sales', 0, 0, 'L',false,'');
$pdf->Ln();
